...
def comando():
    ...
    try:
        ...
        # Leggi lo stato attuale dei sensori
        DR_status = GPIO.input(DR)
        DL_status = GPIO.input(DL)
        
        # Converti i valori GPIO in booleani per il JSON
        # Nota: con PUD_UP, 0 indica rilevamento (attivo) e 1 indica nessun rilevamento
        sensore_destro = DR_status == 0
        sensore_sinistro = DL_status == 0
            
        if action == 'avanti':
            if sensore_sinistro == 0 and sensore_destro == 0:
                bot.forward()
...
@app.route('/api/sensori', methods=['GET', 'POST'])
def statosensori():
    GPIO.setup(DR, GPIO.IN, GPIO.PUD_UP)
    GPIO.setup(DL, GPIO.IN, GPIO.PUD_UP)
    ...
    return jsonify({
        'sensore_sinistro': sensore_sinistro,
        'sensore_destro': sensore_destro
    })
